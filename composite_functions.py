...
def adicionar_ID_partidas_em_csv_para_time(nome_do_time, jogadores, data_folder, dia_inicial_em_epoch):
    path = os.getcwd()
    data_folder_path = os.path.join(path, data_folder)

    time_pasta = os.path.join(os.path.abspath(data_folder_path), nome_do_time)
    
    if not os.path.exists(time_pasta):
        os.makedirs(time_pasta)
...
def atualizar_partidas_de_todos_jogadores(times, data_folder):
    ...
    for time in times:
        time_pasta = os.path.join(os.path.abspath(data_folder_path), time)
            ...
            if player_folder_exists:
                with open(lista_matches, "r") as file: 
                    ...
                    for partida in contents:
                        if not os.path.isfile(os.path.join(time_pasta,player, partida)):
...
#data_folder é o path completo da pasta em que os dados estão, menos a pasta com o nome do jogador
def search_for_champs_played_in_folder_of_matches(player_name, player_uuid, data_folder):
    #mudar o diretoria que a função executa para data_folder
    ...
    for game_id in file_paths:
        ...
        if game["info"]["gameDuration"] >= tempo_minimo_de_jogo:
            if game["info"]["gameMode"] == "CLASSIC": #Modos não classicos não tem challanges
                for participant in game['info']["participants"]:
                    if participant["summonerName"] == player_name or participant["puuid"] == player_uuid: #se o player tiver mudado de nome, buscar por puuid
    ...
    #salvando todos os campẽos com que jogou
    file_name = player_name + "_campeões_jogados.json"
    champions_played_ordenados = dict(sorted(champions_played.items(), key = lambda x: x[1]["Partidas"], reverse=True))
    with open(file_name, 'w') as cpo:
        json.dump(champions_played_ordenados, cpo)

    #criando um dossie com os campeões mais relevantes do player
